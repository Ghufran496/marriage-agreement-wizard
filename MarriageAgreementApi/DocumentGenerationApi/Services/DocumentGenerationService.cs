...
namespace DocumentGenerationApi.Services
{
    public class DocumentGenerationService
    {
        ...
        private static string DefaultDate = "January 1, 2000";
        ...
        public byte[] GenerateSeparationAgreement(DocumentRequest request)
        {
            using (MemoryStream memoryStream = new MemoryStream())
            {
                    ...
                    WordprocessingDocument.Create(memoryStream, WordprocessingDocumentType.Document))
                {
                    ...
                    AddHeading(body, "MARRIAGE AGREEMENT", 1, true, true);
                    AddSpacing(body, 24);
                    ...
                    AddParagraphWithAllCaps(body, "THIS IS A MARRIAGE AGREEMENT DATED ___________ DATE OF SIGNING", false);
                    AddSpacing(body, 12);
                    ...
                    AddSpacing(body, 12);
                    AddFooter(body, $"This document was generated using Marriage Agreement Wizard on {DateTime.Now:MMMM d, yyyy}");
                }
        ...
        private void AddDifferentFirstPageHeader(WordprocessingDocument wordDocument)
        {
            ...
            leftRun.AppendChild(new Text("Marriage Agreement"));
            ...
            GridSpan gridSpan = new GridSpan() { Val = 2 };
        ...
        private void AddHorizontalLine(Body body)
        {
            ...
            // Create paragraph border properties
            ParagraphProperties paragraphProperties = new ParagraphProperties();
            ParagraphBorders borders = new ParagraphBorders();
            
            // Use TopBorder instead of BorderTop
        ...
        private string FormatClauseText(string text, PartyInfo partyInfo)
        {
            ...
            string marriageDate = partyInfo.MarriedDate.HasValue 
                ? partyInfo.MarriedDate.Value.ToString("MMMM d, yyyy") 
                : DefaultDate;
                
            string cohabitationDate = partyInfo.CohabitationDate.HasValue 
                ? partyInfo.CohabitationDate.Value.ToString("MMMM d, yyyy") 
                : DefaultDate;
                
            result = result.Replace("[marriageDate]", marriageDate);
            result = result.Replace("[cohabitationDate]", cohabitationDate);
